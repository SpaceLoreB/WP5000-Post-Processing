function [sideResults] = processSide(velComponents)
% if nargin > 1
...
% search/replace...
q_tot = sum(rawVelocities,2)*36; % total (after bkg noise cutoff)
...
sideResults.vMax = max(rawVelocities,[],2);  % no need to carry over the usable: they're just the ones > vMin
end